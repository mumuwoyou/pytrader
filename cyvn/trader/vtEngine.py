...
from cyvn.trader.language import text
...
class MainEngine(object):
    ...
    #----------------------------------------------------------------------
    def addGateway(self, gatewayName):
        """添加底层接口"""
        """添加底层接口"""
        #gatewayName = 'CTP' #gatewayModule.gatewayName

        # 创建接口实例
        self.gatewayDict[gatewayName] = CtpGateway(self.eventEngine, gatewayName) #gatewayModule.gatewayClass(self.eventEngine, gatewayName)

        # 设置接口轮询
        #if gatewayModule.gatewayQryEnabled:
        self.gatewayDict[gatewayName].setQryEnabled(True)#gatewayModule.gatewayQryEnabled)

        # 保存接口详细信息
        d = {
            'gatewayName': gatewayName,
            'gatewayDisplayName': gatewayName,#gatewayModule.gatewayDisplayName,
            'gatewayType': 'futures' #vtConstant.GATEWAYTYPE_FUTURES #gatewayModule.gatewayType
        }
    ...
    def dbConnect(self):
        ...
        if not self.dbClient:
            ...
            try:
                ...
                if globalSetting['mongoPwd'] != "":
                    self.dbClient = MongoClient(globalSetting['mongoHost'],
                                                globalSetting['mongoPort'],
                                                username = globalSetting['mongoUser'],
                                                password = globalSetting['mongoPwd'],
