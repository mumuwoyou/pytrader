...
# 创建主引擎代理对象
from cyvn.event import EventEngine2
from cyvn.trader.vtEvent import (EVENT_TICK, EVENT_ORDER, EVENT_TRADE,
                                 EVENT_ACCOUNT, EVENT_POSITION, EVENT_LOG,
                                 EVENT_ERROR, EVENT_CONTRACT)
from cyvn.trader.vtObject import VtSubscribeReq, VtOrderReq, VtCancelOrderReq
from cyvn.trader.app.rpcService.rsClient import MainEngineProxy
from cyvn.trader.app.ctaStrategy.ctaBase import EVENT_CTA_LOG, EVENT_CTA_STRATEGY
...
TODAY = str(datetime.datetime.now().date()).encode(encoding='utf-8')

with open("WEB_setting.json") as f:
    setting = json.load(f)
    USERNAME = str(setting['username']).encode(encoding='utf-8')
    PASSWORD = str(setting['password']).encode(encoding='utf-8')
    TOKEN = base64.encodebytes(TODAY+PASSWORD).replace('\n'.encode(encoding='utf-8'),''.encode(encoding='utf-8')).decode(encoding='utf-8')
...
class Token(Resource):
    ...
    def get(self):
        ...
        username = str(args['username']).encode(encoding='utf-8')
        password = str(args['password']).encode(encoding='utf-8')
